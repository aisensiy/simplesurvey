...
class Errors implements IteratorAggregate
{
	 ...
	 * @return array
	 */
	public function full_messages($alias=array())
	{
		$full_messages = array();
	 ...
	 * @return array
	 */
	public function to_array($closure=null, $alias=array())
	{
		$errors = array();
		...
		if ($this->errors)
		{
			foreach ($this->errors as $attribute => $messages)
			{
				foreach ($messages as $msg)
				{
					...
					$errors[$attribute][] = ($message = (!empty($alias[$attribute]) ? $alias[$attribute] : Utils::human_attribute($attribute)) . ' ' . $msg);
